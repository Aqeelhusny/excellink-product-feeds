...
class ELF_Admin_Page {
    ...
    public static function register_menu(): void {
        add_menu_page(
            __( 'Product Feeds', 'excellink-feeds' ),
            __( 'Product Feeds', 'excellink-feeds' ),
            'manage_options',
            self::MENU_SLUG,
        ...
        add_submenu_page(
            self::MENU_SLUG,
            __( 'Category Mapping', 'excellink-feeds' ),
            __( 'Category Mapping', 'excellink-feeds' ),
            'manage_options',
            'elf-category-map',
    ...
    public static function enqueue_assets( string $hook ): void {
        ...
        wp_localize_script( 'elf-admin', 'elfData', [
            ...
            'i18n'     => [
                'saving'       => __( 'Saving…', 'excellink-feeds' ),
                'saved'        => __( 'Saved!', 'excellink-feeds' ),
                'generating'   => __( 'Generating feeds…', 'excellink-feeds' ),
                'generated'    => __( 'Feeds generated!', 'excellink-feeds' ),
                'error'        => __( 'An error occurred. Please try again.', 'excellink-feeds' ),
                'loading'      => __( 'Loading taxonomy…', 'excellink-feeds' ),
                'exporting'    => __( 'Exporting…', 'excellink-feeds' ),
                'importing'    => __( 'Importing…', 'excellink-feeds' ),
                'imported'     => __( 'Settings imported!', 'excellink-feeds' ),
                'exported'     => __( 'Settings exported!', 'excellink-feeds' ),
            ],
        ]);
    }
    ...
    public static function render_settings(): void {
        if ( ! current_user_can( 'manage_options' ) ) {
            wp_die( esc_html__( 'Insufficient permissions.', 'excellink-feeds' ) );
        }
    ...
    public static function ajax_save_settings(): void {
        ...
        if ( ! current_user_can( 'manage_options' ) ) {
            wp_send_json_error( [ 'message' => __( 'Insufficient permissions.', 'excellink-feeds' ) ] );
        }
        ...
        $settings = [
            ...
            'batch_size'           => absint( $_POST['batch_size'] ?? 200 ),
        ...
        wp_send_json_success( [ 'message' => __( 'Settings saved.', 'excellink-feeds' ) ] );
    }
    ...
    public static function ajax_regenerate_feeds(): void {
        ...
        // Set higher time limit for large catalogs
        ...
        if ( $feed_ok ) {
            ...
            wp_send_json_success( [
                'message' => sprintf(
                    __( 'Feed and image sitemap regenerated successfully in %s seconds.', 'excellink-feeds' ),
                    $duration
                ),
                'duration' => $duration,
            ]);
            ...
            wp_send_json_error( [ 'message' => __( 'Feed generation failed. Check that products exist and are published.', 'excellink-feeds' ) ] );
        }
    }
    ...
    public static function ajax_save_category_map(): void {
        ...
        $raw_map = $_POST['category_map'] ?? [];

        if ( ! is_array( $raw_map ) ) {
            wp_send_json_error( [ 'message' => __( 'Invalid data.', 'excellink-feeds' ) ] );
        }
        ...
        wp_send_json_success( [ 'message' => __( 'Category mapping saved.', 'excellink-feeds' ) ] );
    }
    ...
    public static function ajax_search_taxonomy(): void {
        ...
        if ( empty( $taxonomy ) ) {
            wp_send_json_error( [ 'message' => __( 'Could not fetch Google taxonomy. Check your internet connection.', 'excellink-feeds' ) ] );
        }
    ...
    public static function ajax_import_settings(): void {
        ...
        if ( empty( $settings_json ) ) {
            wp_send_json_error( [ 'message' => __( 'No settings data provided.', 'excellink-feeds' ) ] );
        }

        $settings = json_decode( $settings_json, true );
        
        if ( json_last_error() !== JSON_ERROR_NONE ) {
            wp_send_json_error( [ 'message' => __( 'Invalid settings data format.', 'excellink-feeds' ) ] );
        }
        ...
        wp_send_json_success( [ 'message' => __( 'Settings imported successfully.', 'excellink-feeds' ) ] );
    }
    ...
    public static function ajax_clear_logs(): void {
        ...
        wp_send_json_success( [ 'message' => __( 'Logs cleared successfully.', 'excellink-feeds' ) ] );
    }
}
